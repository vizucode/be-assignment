...
type IRepoDatabase interface {
	...
	FindUserAccount(ctx context.Context, userAccountCode string) (resp models.UserAccount, err error)
	CreateTransaction(ctx context.Context, transaction models.Transaction) (resp models.Transaction, err error)
	CheckOwnerAccount(ctx context.Context, userId int, accountCode string) (resp models.UserAccount, err error)
}
